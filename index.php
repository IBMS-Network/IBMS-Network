...
try {
    ...
    if (class_exists('classes\core\clsCore')) {
        $objCore = new classes\core\clsCore();
        $objCore->runApp();
    } else {
        $throw_mess = '<br> Class [clsCore] is not init.<br>';
        throw new \Exception($throw_mess);
    }
} catch (\Exception $e) {
...
{
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "Googlebot/2.1") !== false) {
        return false;
    }
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "msnbot") !== false) {
        return false;
    }
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "Alexa") !== false) {
        return false;
    }
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "Yahoo") !== false) {
        return false;
    }
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "Ask/Teoma") !== false) {
        return false;
    }
    if (isset($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], "Jeeves") !== false) {
        return false;
    }
    return true;
}