...
class Subsys_JsHttpRequest_Php
{
     ...
     */
    public function Subsys_JsHttpRequest_Php($enc)
    {
    ...
    public function getJsCode()
    {
		...
		return file_get_contents( localRoot."/js/ajax.js" );
    }
	...
	{
		$this->OUT_RESULT = $_RESULT;
		//file_put_contents( "out_res.txt", serialize( $this->OUT_RESULT ) );
	}
    ...
    public function _obHandler($text)
    {
		...
		if( @sizeof( $this->OUT_RESULT ) < 1 )
		{
			$this->OUT_RESULT = array(
			  "q"     => $q,
			  "md5"   => md5( $q )
			); 
		}
		...
		$result = $this->php2js( $this->OUT_RESULT );
        $text = 
